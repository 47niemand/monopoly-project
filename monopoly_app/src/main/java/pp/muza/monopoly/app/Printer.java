...
public class Printer {
    ...
    static void printResults(PlayGame game) {
        ...
        System.out.println(MessageFormat.format(resourceBundle.getString("FREE_PROPERTIES"), freeProperties));
    ...
    static String cardInfo(String playerName, Board board, ActionCard card) {
        if (card instanceof PayRent) {
            ...
            return AppTexts.formatMessage(AppTexts.RENT_MSG, Map.of(
            ...
            Buy buy = (Buy) card;
            return AppTexts.formatMessage(AppTexts.BUY_MSG, Map.of(
    ...
    private static void paymentMsg(String playerName, Board board, List<Debt> paymentCards, StringBuilder sb) {
        ...
        if (r != null) {
            ...
            if (spending - r.getValue() > 0) {
                sb.append(AppTexts.formatMessage(AppTexts.AND_OTHER_DEBTS_MSG, Map.of(
                        "player", playerName,
                        "amount", spending - r.getValue()
                )));
            }
        } else {
            sb.append(AppTexts.formatMessage(AppTexts.PLAYER_PAYS_DEBTS_MSG, Map.of(
                    "player", playerName,
                    "amount", spending
            )));
        }
    ...
    private static void incomeMsg(String playerName, List<Income> incomeCards, StringBuilder sb) {
        ...
        if (incoming - rent > 0) {
            if (r.size() > 0) {
                sb.append(AppTexts.formatMessage(AppTexts.OTHER_INCOME_MSG, Map.of(
                        "player", playerName,
                        "amount", incoming - rent
                )));
            } else {
                sb.append(AppTexts.formatMessage(AppTexts.INCOME_MSG, Map.of(
                        "player", playerName,
                        "amount", incoming
                )));
            }
