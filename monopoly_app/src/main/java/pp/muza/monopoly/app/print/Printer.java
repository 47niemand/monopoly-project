...
import pp.muza.monopoly.model.pieces.actions.BaseBuy;
...
public class Printer {
    ...
    public static void printResults(PlayGame game) {
        ...
        println(MessageFormat.format(resourceBundle.getString("FREE_PROPERTIES_MSG"), freeProperties));
        ...
        // print results
        game.getPlayers().forEach(x -> println(String.format("%s - %s", x.getName(), game.getPlayerInfo(x))));
    }
    ...
    public static void printTurnInfo(Board board, TurnInfo turnInfo) {
        ...
        List<BaseBuy> buyCards = cards.stream().filter(x -> x.getAction() == Action.BUY).map(x -> (BaseBuy) x)
                .collect(Collectors.toList());
        List<Fortune> fortuneCards = cards.stream().filter(x -> x.getAction() == Action.CHANCE).map(x -> (Fortune) x)
                .collect(Collectors.toList());
        List<Contract> contractCards = cards.stream().filter(x -> x.getAction() == Action.CONTRACT)
                .map(x -> (Contract) x).collect(Collectors.toList());
        List<Income> incomeCards = cards.stream().filter(x -> x.getAction() == Action.INCOME).map(x -> (Income) x)
                .collect(Collectors.toList());
        List<BaseDebt> paymentCards = cards.stream().filter(x -> x.getAction() == Action.DEBT).map(x -> (BaseDebt) x)
                .collect(Collectors.toList());
        List<Arrival> arrivalCards = cards.stream().filter(x -> x.getAction() == Action.ARRIVAL).map(x -> (Arrival) x)
                .collect(Collectors.toList());
        StringBuilder sb = new StringBuilder();

        playedTurnMsg(turnNumber, playerName, turnInfo.getPlayerInfo(), sb);
        if (!incomeCards.isEmpty()) {
    ...
    static String cardInfo(String playerName, Board board, ActionCard card) {
        if (card instanceof PayRent rent) {
        ...
        } else if (card instanceof BaseBuy buy) {
        ...
        } else if (card instanceof Fortune fortune) {
    ...
    static void playerStatus(Board board, TurnInfo turnInfo, String playerName, StringBuilder sb) {
        if (turnInfo.getPlayerInfo().getStatus() == PlayerStatus.OUT_OF_GAME) {
            String obligatoryCards = turnInfo.getActiveCards().stream()
                    .map(actionCard -> cardInfo(playerName, board, actionCard)).collect(Collectors.joining(", "));
            sb.append(obligatoryCards).append(LINE_BREAK);
            sb.append(format(OUT_OF_GAME_MSG, Map.of(
                    "player", playerName))).append(LINE_BREAK);
    ...
    static void paymentMsg(String playerName, Board board, List<BaseDebt> paymentCards, StringBuilder sb) {
        ...
        PayRent r = paymentCards.stream().filter(x -> x instanceof PayRent).map(x -> (PayRent) x).findFirst()
                .orElse(null);
        if (r != null) {
    ...
    static void incomeMsg(String playerName, List<Income> incomeCards, StringBuilder sb) {
        List<RentRevenue> r = incomeCards.stream().filter(x -> x instanceof RentRevenue).map(x -> (RentRevenue) x)
                .collect(Collectors.toList());
        Integer rent = 0;
        if (!r.isEmpty()) {
            String sender = r.stream().map(RentRevenue::getSender).map(Player::getName)
    ...
    static void lostMsg(Board board, String playerName, List<Contract> contractCards, StringBuilder sb) {
        List<Property> lostProperties = contractCards.stream()
                .map(actionCard -> board.getLand(actionCard.getPosition())).map(x -> (Property) x)
                .collect(Collectors.toList());
        ...
        if (lostProperties.size() == 1) {
            sb.append(format(LOST_PROPERTY_MSG, Map.of(
                    "player", playerName,
                    "land", s))).append(LINE_BREAK);
        } else {
            sb.append(format(LOST_PROPERTY_PLURAL_MSG, Map.of(
                    "player", playerName,
                    "lands", s))).append(LINE_BREAK);
        }
    }

    static void newOwnsMsg(Board board, String playerName, List<BaseBuy> buyCards, StringBuilder sb) {
        List<Property> ownedProperties = buyCards.stream().map(actionCard -> board.getLand(actionCard.getPosition()))
                .map(x -> (Property) x).collect(Collectors.toList());
    ...
    static void playedCardsMsg(String playerName, List<ActionCard> cards, StringBuilder sb) {
        String cardsPlayed = !cards.isEmpty()
                ? cards.stream().map(ActionCard::getName).map(i18n()).collect(Collectors.joining(", "))
                : "none";
        sb.append(format(PLAYED_CARDS_MSG, Map.of(
                "player", playerName,
                "cards", cardsPlayed))).append(LINE_BREAK);
    }

    static void playerLocationMsg(String playerName, int positionNumber, Board board, Land land,
            List<Arrival> arrivalCards, StringBuilder sb) {
        sb.append(format(PLAYED_TURN_LOCATION_MSG, Map.of(
                "land", resourceBundle.getString(land.getName()),
                "position", positionNumber,
                "player", playerName)));
        if (arrivalCards.size() > 1) {
            List<Land> landsExcludingLast = arrivalCards.stream().map(Arrival::getPosition)
                    .filter(x -> x != positionNumber).map(board::getLand).collect(Collectors.toList());
    ...
    static void playedTurnMsg(int turnNumber, String playerName, PlayerInfo playerInfo, StringBuilder sb) {
        sb.append(format(PLAYED_TURN_MSG, Map.of(
                "number", turnNumber,
                "player", playerName))).append(" ").append(format(PLAYER_INFO, Map.of(
                        "player", playerName,
